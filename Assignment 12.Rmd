...
- Create a list of pairs of words that may contain stop words and plot the most frequent pairs.
...
#Identify words that connect with not, no, never, without... then reverse the sentiment value of these words
...
Question 2. Do Question 1 on your own text data set.
...
#Create list of pairs of words
df_bigrams <- df %>%
  unnest_tokens(input = highlights, output = bigram, token = "ngrams", n = 2) %>% 
  filter(!is.na(bigram))
...
sentiment1 = df %>%
  unnest_tokens(input = highlights, output = word) %>% 
  anti_join(get_stopwords()) %>% 
  anti_join(stop_word2) %>% 
  inner_join(get_sentiments("afinn")) %>% 
  filter(!is.na(value)) %>% 
